...
export function Step8Shortlist({ selectedTasks, onNext, onBack }: Step8Props) {
  ...
  const topCandidates = CANDIDATES
    .filter(c => c.overallScore >= 90 && !disqualifiedCandidates.has(c.id))
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col">
        <div className="text-center mb-3">
          ...
          <p className="text-xs font-light text-gray-600">
            {topCandidates.length} candidates scored ≥90% — ranked by performance
          </p>
        </div>
          ...
          {starredCandidates.size > 0 && (
            <Card className="p-4 bg-yellow-50 border-2 border-yellow-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-light text-gray-900 text-sm mb-0.5">
                    {starredCandidates.size} candidate{starredCandidates.size !== 1 ? 's' : ''} starred
                  </div>
